(() => {
  ...
  function renderCatalog() {
    ...
    try {
      ...
      state.history = Array.isArray(response.history)
        ? response.history
        : [];
  ...
  async function goDashboard(message = '') {
    ...
    try {
      ...
      toast(
        `Histori belum bisa dimuat: ${error.message}`,
        'warning',
        4200
      );
    }
  ...
  async function requestReset() {
    ...
    if (!identifier) {
      error.textContent =
        'Masukkan username atau email dulu.';
    ...
    try {
      ...
      success.textContent =
        response.message ||
        'Kalau akun ditemukan, link reset sudah dikirim ke email terdaftar.';
  ...
  async function logout() {
    try {
      ...
      // meskipun request backend gagal.
    } finally {
      clearSession();
      leaveGuest();
        ...
        .innerHTML = `
          ...
          <div class="instruction-grid">
            <div class="tip">
              <b>50 kolom</b>
              <small>
                Setiap kolom memiliki 26 jawaban.
              </small>
            </div>
            ...
            <div class="tip">
              <b>15 detik</b>
              <small>
                Waktu otomatis berpindah ke kolom berikutnya.
              </small>
            </div>
          </div>
        `;
        ...
        .innerHTML = `
          <div class="instruction-grid">
            <div class="tip">
              <b>20 soal</b>
              <small>
                Setiap paket menggunakan bank soal JSON yang sesuai.
              </small>
            </div>
            ...
            <div class="tip">
              <b>30 detik/soal</b>
              <small>
                Timer otomatis berpindah bila waktu habis.
              </small>
            </div>
            ...
            <div class="tip">
              <b>3 paket</b>
              <small>
                Paket 1–3 memiliki bank soal masing-masing.
              </small>
            </div>
            ...
            <div class="tip">
              <b>Hasil PDF</b>
              <small>
                Hasil tetap dapat diunduh meskipun mode tamu.
              </small>
            </div>
          </div>
        `;
  ...
  async function loadQuestionPackage(
    testId,
    packageNumber
  ) {
    const filePath =
      QUESTION_FILES[testId];
    ...
    const selectedPackage =
      data.paket.find(
        (item) =>
          Number(item.id_paket) ===
          Number(packageNumber)
      );

    if (!selectedPackage) {
      throw new Error(
        `Paket ${packageNumber} untuk ${
          TESTS[testId].name
        } tidak ditemukan.`
      );
    }
    ...
    if (
      !Array.isArray(
        selectedPackage.soal
      ) ||
      selectedPackage.soal.length !==
    ...
    return selectedPackage.soal.map(
      (item) => {
        if (
          !item.question ||
          !item.options ||
          ...
          throw new Error(
            `Struktur soal nomor ${item.id} tidak lengkap.`
          );
        }
        ...
        if (
          ...
          throw new Error(
            `Soal nomor ${item.id} tidak memiliki pilihan jawaban yang cukup.`
          );
        }
        ...
        if (
          ...
          throw new Error(
            `Jawaban soal nomor ${item.id} tidak ditemukan.`
          );
        }
  ...
  function renderMCQ() {
    ...
    $('testHint')
      .textContent =
      `Pilih satu jawaban. Waktu per soal: ${CONFIG.MCQ_SECONDS} detik.`;
  }
  ...
  function calculateMCQ() {
    ...
    const correct = state.answers.reduce(
      (sum, answer, index) =>
        sum +
        (
          answer === state.questions[index].correct
            ? 1
            : 0
        ),
      0
  ...
  function resumePersistedTest() {
    ...
    if (!saved) {
      toast(
        'Tidak ada progress tes yang bisa dilanjutkan.',
        'warning'
      );
      return;
    }
    ...
    state.colIndex =
      Number(saved.colIndex) || 0;

    state.qIndex =
      Number(saved.qIndex) || 0;
  ...
  async function saveHistory(result) {
    ...
    try {
      ...
      toast(
        `Hasil tampil, tetapi belum masuk spreadsheet: ${error.message}`,
        'warning',
        5000
      );
  ...
  async function finishTest() {
    ...
    if (!state.isGuest) {
      try {
        ...
        toast(
          'Hasil tersimpan ke Google Sheets.',
          'success'
        );
      } catch (error) {
        console.warn(
          'Refresh histori gagal:',
          error
        );
      }
    }
  ...
  function renderResult(result) {
    const name = state.isGuest
      ? 'Tamu'
      : state.session?.username ||
        'Peserta';

    $('resultTitle').textContent =
      `${TESTS[result.type].name} selesai 🎉`;
  ...
  function abandonTest() {
    ...
    toast(
      'Tes dibatalkan. Progress tidak disimpan.',
      'info'
    );
  }
  ...
  async function init() {
    ...
    if (state.session) {
      ...
      try {
        ...
        toast(
          `Belum dapat mengambil histori: ${error.message}`,
          'warning',
          4500
        );
      }
      ...
      if (loadPersistedTest()) {
        toast(
          'Ada progress tes yang tersimpan. Kamu bisa melanjutkannya dari sesi ini.',
          'info',
          4200
        );
      }
