(() => {
  ...
  // POST -> hidden iframe
  // Response -> postMessage from Apps Script HTML response
  // ============================================================
  ...
  function randomToken(prefix = '') {
    const part = () => Math.floor(Math.random() * 0xffffff).toString(16).padStart(6, '0');
    return `${prefix}${Date.now().toString(36)}-${part()}${part()}${part()}`;
  }
  ...
  window.addEventListener('message', (event) => {
    ...
    if (!message || message.type !== 'PSYCHOTEST_API_RESPONSE' || !message.nonce) return;
    ...
    if (message.ok === false) {
      waiter.reject(new Error(message.error || 'Backend gagal memproses permintaan.'));
      return;
    }
  ...
  function api(action, payload = {}, timeoutMs = 20000) {
    return new Promise((resolve, reject) => {
      ...
      dataInput.name = 'data';
      dataInput.value = JSON.stringify({ action, ...payload, nonce });
      ...
      const timeoutId = setTimeout(() => {
        ...
        form.remove();
        reject(new Error('Backend tidak merespons dalam 20 detik.'));
      }, timeoutMs);
  ...
  function ensurePdfLibrary() {
    ...
    pdfPromise = (async () => {
      ...
      throw new Error('Library PDF tidak dapat dimuat dari CDN.');
    })().catch((error) => {
      pdfPromise = null;
      throw error;
    });
  ...
  function answerMCQ(choice) {
    ...
    if (choice !== null) {
      ...
      buttons.forEach((button, buttonIndex) => {
        ...
        if (buttonIndex === choice && choice !== question.correct) button.classList.add('wrong');
      });
    }

    persistTest();

    const delay = choice === null ? 0 : 180;
    setTimeout(() => {
  ...
  async function downloadPdf() {
    ...
    try {
      ...
      doc.setFontSize(20);
      doc.text('Hasil Latihan Psikotes', 20, 22);

      doc.setFont('helvetica', 'normal');
      doc.setFontSize(10);
      doc.text(`Peserta: ${name}`, 20, 31);
      doc.text(`Tes: ${TESTS[result.type].name} • Paket ${result.package}`, 20, 37);
      doc.text(`Tanggal: ${new Date(result.tanggal).toLocaleString('id-ID')}`, 20, 43);

      doc.setFont('helvetica', 'bold');
      doc.setFontSize(13);
      doc.text('Ringkasan Performa', 20, 56);
      ...
      rows.forEach(([label, value], index) => {
        doc.text(`${label}: ${value}`, 20, 68 + index * 7);
      });

      const chart = $('performanceChart');
      if (chart) {
        doc.addImage(chart.toDataURL('image/png'), 'PNG', 20, 132, 170, 58);
      }

      doc.setFontSize(8.5);
      doc.setTextColor(100, 112, 128);
      const disclaimer =
        'Catatan: skor ini adalah indikator latihan internal, bukan norma resmi psikotes, bukan diagnosis psikologis, dan bukan keputusan kelulusan seleksi.';
      doc.text(doc.splitTextToSize(disclaimer, 170), 20, 200);

      doc.setTextColor(24, 38, 59);
      doc.setFontSize(10);
      doc.text('Interpretasi latihan', 20, 220);
      doc.setFontSize(9);
      doc.text(`Kecepatan: ${level(result.speed)} • Ketelitian: ${level(result.accuracy)}`, 20, 228);
      doc.text(`Konsistensi: ${level(result.consistency)} • Ketahanan: ${level(result.endurance)}`, 20, 235);

      const safeType = result.type.replace(/[^a-z0-9-]/gi, '-');
      doc.save(`hasil-${safeType}-paket-${result.package}-${new Date().toISOString().slice(0, 10)}.pdf`);
      toast('PDF berhasil dibuat.', 'success');
    } catch (error) {
      console.error(error);
      toast('PDF gagal dibuat. Pastikan internet aktif agar library PDF dapat dimuat.', 'warning', 5000);
    } finally {
      busy(button, '', false);
    }
  }
